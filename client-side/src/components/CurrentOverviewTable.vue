<template>
  <div
    id="poppins"
    class="border p-3 col-span-2 border-[#28361822] rounded-lg shadow-lg"
  >
    <div class="flex justify-between items-center">
      ...
      <div class="flex flex-col items-end gap-1">
        ...
        <div class="w-full md:w-96">
          <div
            class="flex items-center gap-2 border rounded-lg border-[#4B5D0E66] bg-white px-3 py-2 shadow-sm focus-within:border-[#283618]"
          >
            ...
            <button
              v-if="searchQuery"
              @click="searchQuery = ''"
              class="text-[#283618]/60 hover:text-[#283618]"
    ...
    <div class="flex justify-between items-center gap-5 mt-8">
      ...
      <div>
        <ion-button
          @click="handleShowAddCustomer"
          :disabled="addDisabled"
          class="normal-case bg-[#283618] text-white rounded-sm"
          :title="addDisabled ? 'Max queue length reached' : ''"
    ...
    <div v-if="customerTab === 'current'">
      <div class="my-8 border-gray-200 shadow-sm rounded-lg">
        <div class="table-scroll-wrapper overflow-x-auto">
          <table class="queue-table w-full min-w-max">
            ...
            <tbody class="border-gray-400 text-[#283618]">
              <tr
                v-for="(customer, index) in pagedCurrentCustomers"
                :key="customer.id || customer.queueNo || index"
              >
                ...
                <td class="!p-3">
                  <div class="flex items-center gap-2">
                    ...
                    <span
                      v-if="customer.paid"
                      class="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-emerald-100 text-emerald-700"
                      >Paid</span
                    >
                  </div>
                </td>
                <td class="!p-3">
                  <span class="flex items-center justify-center gap-2">
                    <font-awesome-icon
                      @click="handleOrderDetails(customer)"
                      :icon="['fas', 'eye']"
                      class="text-sm bg-[#FEFAE080] hover:opacity-75 border border-[#00000033] py-1 px-3 rounded-lg cursor-pointer"
                    />
                    <font-awesome-icon
                      :icon="['fas', 'check']"
                      @click="
                        isServeDisabled(customer)
                          ? null
                      ...
                      :class="[
                        ...
                        isServeDisabled(customer)
                          ...
                          : 'text-[#40CF3E] hover:opacity-75 cursor-pointer',
                      ]"
                      :title="
                        isServeDisabled(customer)
                          ? 'Approve payment and call the order before serving'
                          : 'Mark as served'
                      "
                    />

                    <span
                      v-if="String(customer.status).toLowerCase() === 'delayed'"
                      class="relative group"
                      aria-label="Customer requested delay"
                    >
                      <font-awesome-icon
                        :icon="['fas', 'clock']"
                        class="text-orange-600 text-base cursor-default"
                      />
                      <span
                        class="absolute z-10 hidden group-hover:block -top-9 left-1/2 -translate-x-1/2 whitespace-nowrap bg-white text-[#283618] text-[10px] px-2 py-1 rounded shadow border"
                        >Customer requested delay</span
                      >
                    </span>
                  </span>
                </td>
      ...
      >
        <div class="text-xs text-[#283618]/70">
          Page {{ currentPageCurrent }} of {{ currentTotalPages }}
        </div>
        <div class="flex items-center gap-1 flex-wrap">
          <ion-button
            ...
            @click="currentPageCurrent = Math.max(1, currentPageCurrent - 1)"
            >Prev</ion-button
          >
          <ion-button
            ...
            :key="'c' + page"
            size="small"
            :fill="page === currentPageCurrent ? 'solid' : 'clear'"
            @click="currentPageCurrent = page"
            >{{ page }}</ion-button
          >
          <ion-button
            size="small"
            fill="clear"
            :disabled="currentPageCurrent >= currentTotalPages"
            @click="
              currentPageCurrent = Math.min(
                currentTotalPages,
      ...
      <div v-else-if="searchQuery" class="text-sm text-gray-500 mt-2">
        No matching results
      </div>
    </div>
      ...
      <div class="my-8">
        <div class="table-scroll-wrapper overflow-x-auto">
          <table class="queue-table w-full min-w-max rounded-lg">
            ...
            <tbody class="border-gray-400 text-[#283618]">
              <tr
                v-for="(customer, index) in pagedCompletedCustomers"
                :key="index"
              >
                ...
                <td class="!p-3 text-sm font-light">
                  <span
                    class="border border-blue-300 text-[#1E589A] rounded-xl px-2 font-medium"
                    >{{ customer.status }}</span
                  >
                </td>
                <td class="!p-3 text-sm font-light">
                  <span
                    class="bg-[#38C536] text-white rounded-xl px-2 font-medium"
        ...
        >
          <div class="text-xs text-[#283618]/70">
            Page {{ currentPageCompleted }} of {{ completedTotalPages }}
          </div>
          <div class="flex items-center gap-1 flex-wrap">
            <ion-button
              size="small"
              ...
              @click="
                currentPageCompleted = Math.max(1, currentPageCompleted - 1)
              "
              >Prev</ion-button
            >
            <ion-button
              ...
              :key="'d' + page"
              size="small"
              :fill="page === currentPageCompleted ? 'solid' : 'clear'"
              @click="currentPageCompleted = page"
              >{{ page }}</ion-button
            >
            <ion-button
              size="small"
              fill="clear"
              :disabled="currentPageCompleted >= completedTotalPages"
              @click="
                currentPageCompleted = Math.min(
                  completedTotalPages,
      ...
      <div class="my-8">
        <div class="table-scroll-wrapper overflow-x-auto">
          <table class="queue-table w-full min-w-max rounded-lg">
            ...
            <tbody class="border-gray-400 text-[#283618]">
              <tr
                v-for="(customer, index) in pagedCancelledCustomers"
                :key="index"
              >
                <td class="!p-3 text-sm font-light">{{ customer.queueNo }}</td>
                <td class="!p-3 text-sm font-light">
                ...
                <td class="!p-3 text-sm font-light">
                  <span
                    class="border border-red-300 text-red-700 rounded-xl px-2 font-medium"
                    >{{ customer.status }}</span
                  >
                </td>
                <td class="!p-3 text-sm font-light">
                  <span
                    class="bg-gray-300 text-gray-800 rounded-xl px-2 font-medium"
                    >{{ customer.payment }}</span
                  >
                </td>
        ...
        <div
          class="flex items-center justify-between mt-2"
          v-if="filteredCancelled.length > 0"
        >
          <div class="text-xs text-[#283618]/70">
            Page {{ currentPageCancelled }} of {{ cancelledTotalPages }}
          </div>
          <div class="flex items-center gap-1 flex-wrap">
            <ion-button
              size="small"
              ...
              @click="
                currentPageCancelled = Math.max(1, currentPageCancelled - 1)
              "
              >Prev</ion-button
            >
            <ion-button
              v-for="page in cancelledPages"
              :key="'x' + page"
              size="small"
              :fill="page === currentPageCancelled ? 'solid' : 'clear'"
              @click="currentPageCancelled = page"
              >{{ page }}</ion-button
            >
            <ion-button
              size="small"
              fill="clear"
              :disabled="currentPageCancelled >= cancelledTotalPages"
              @click="
                currentPageCancelled = Math.min(
                  cancelledTotalPages,
...
import {
  listQueue as apiListQueue,
  joinQueue as apiJoinQueue,
  updateQueueStatus as apiUpdateQueueStatus,
} from "@/services/api";
  ...
  setup(props, { emit }) {
    ...
    const savedTab =
      typeof window !== "undefined"
        ? localStorage.getItem("overviewActiveTab")
        : null;
    const customerTab = ref(
      savedTab === "completed" || savedTab === "cancelled"
        ? savedTab
        : "current"
    );
    ...
    function loadCache() {
      try {
        ...
        if (raw) customerInfo.value = JSON.parse(raw) || [];
      } catch (e) {
        console.debug("[overview] cache load skipped", e);
      }
    }
    ...
    function saveCache() {
      try {
        ...
        console.debug("[overview] cache save skipped", e);
      }
    }

    try {
      if (CACHE_KEY !== "currentQueueCache")
        sessionStorage.removeItem("currentQueueCache");
    } catch (_) {
      console.debug("[overview] legacy cache key removal skipped");
    }
    ...
    async function loadCurrent() {
      const rows = await apiListQueue({
        status: ["pending", "waiting", "called", "pending_payment", "delayed"],
      });
      try {
        const prevMap = new Map(
          (customerInfo.value || []).map((c) => [c.id, c])
        );
        const mapped = (rows || []).map((r) => {
          const etaBase =
            r.estimated_wait_time != null &&
            !isNaN(Number(r.estimated_wait_time))
              ? Number(r.estimated_wait_time)
              : null;
          const statusLabel =
            r.status === "called"
              ? "In Progress"
          ...
          const canTickNow =
            (stLower === "waiting" || stLower === "delayed") &&
            (props.isServiceBased
              ? true
          ...
          if (prev) {
            ...
            const prevCanTick =
              (prevStatus === "waiting" || prevStatus === "delayed") &&
              (props.isServiceBased ? true : !!prev.paid);

            if (
              sameStatus &&
              prev.initialEwt != null &&
              (etaBase == null || Number(prev.initialEwt) === Number(etaBase))
            ) {
              initialEwt = prev.initialEwt;
              initialStart =
                canTickNow && prevCanTick && prev.initialStart != null
                  ? prev.initialStart
                  : canTickNow
                  ? Date.now()
                  : null;
            ...
            if (currStatus === "in progress") {
              if (typeof prev.frozenWaitSeconds === "number") {
                frozenWaitSeconds = Math.max(
                  0,
                  Math.round(prev.frozenWaitSeconds)
                );
              } else if (
                prev.initialEwt != null &&
                prev.initialStart != null &&
                prevCanTick
              ) {
                const elapsedMin =
                  (Date.now() - Number(prev.initialStart)) / 60000;
                const remSec = Math.max(
                  0,
                  Math.ceil((Number(prev.initialEwt) - elapsedMin) * 60)
                );
        ...
        try {
          ...
          for (const [pid, pin] of pinnedMap.entries()) {
            if (!pin || (pin.expiresAtMs && nowMs > pin.expiresAtMs)) {
              pinnedMap.delete(pid);
              continue;
            }
        ...
        try {
          ...
          for (const [aid, rec] of lastActiveMap.entries()) {
            if (!rec) {
              lastActiveMap.delete(aid);
              continue;
            }
            if (nowMs - (rec.lastSeenMs || 0) > ACTIVE_LEASE_MS) {
              lastActiveMap.delete(aid);
              continue;
            }
        ...
        if (mapped.length === 0) {
          ...
          const optimistic = customerInfo.value.filter(
            (c) => c.optimistic === true
          );
          ...
          for (const opt of optimistic) {
            const dup = merged.some(
              (c) =>
                (opt.id != null && c.id === opt.id) ||
                (opt.queueNo && c.queueNo === opt.queueNo)
            );
          ...
          try {
            if (showOrderDetails.value && selectedOrder.value) {
              ...
              const byId =
                cur.id != null
                  ? customerInfo.value.find((c) => c.id === cur.id)
                  : null;
              const byQn =
                !byId && cur.queueNo
                  ? customerInfo.value.find((c) => c.queueNo === cur.queueNo)
                  : null;
    ...
    async function loadServed() {
      ...
      const servedMapped = (served || []).map((r) => ({
        ...
        joinedAt: r.waiting_at
          ? new Date(r.waiting_at).toLocaleString()
          : r.created_at
          ? new Date(r.created_at).toLocaleString()
          : "-",
        completedAt: r.served_at
      ...
      if (
        currentPageCompleted.value >
        Math.max(1, Math.ceil(completedCustomer.value.length / PAGE_SIZE))
      ) {
        currentPageCompleted.value = Math.max(
          1,
          Math.ceil(completedCustomer.value.length / PAGE_SIZE)
        );
      }
    }
    ...
    async function loadCancelled() {
      ...
      if (Array.isArray(customerInfo.value) && customerInfo.value.length) {
        ...
        if (cancelledIds.size) {
          customerInfo.value = customerInfo.value.filter(
            (c) => !cancelledIds.has(c.id)
          );
          try {
            saveCache();
          } catch (_) {
            console.debug(
              "[overview] unable to persist cache after cancellation"
            );
          }

          try {
            for (const id of cancelledIds.values()) {
              lastActiveMap.delete(Number(id));
              pinnedMap.delete(Number(id));
            }
          } catch (e) {
            console.debug(
              "[overview] unable to cleanup maps after cancellation",
              e
            );
          }
        }
      ...
      if (
        currentPageCancelled.value >
        Math.max(1, Math.ceil(cancelledCustomer.value.length / PAGE_SIZE))
      ) {
        currentPageCancelled.value = Math.max(
          1,
          Math.ceil(cancelledCustomer.value.length / PAGE_SIZE)
        );
      }
    }
    onMounted(async () => {
      ...
      try {
        ...
        console.debug("[overview] initial cache unavailable", e);
      }
      ...
      try {
        ...
        try {
          ...
          if (mappedRows.length === 0) {
            try {
              ...
              console.debug("[overview] cache restore skipped", e);
            }
          } else {
            const merged = typeFiltered.slice();
            try {
              ...
              if (raw) {
                const cached = JSON.parse(raw) || [];
                const optimistic = cached.filter(
                  (c) => c && c.optimistic === true
                );
                for (const opt of optimistic) {
                  if (
                    !merged.some(
                      (c) =>
                        (c.id && opt.id && c.id === opt.id) ||
                        (c.queueNo && opt.queueNo && c.queueNo === opt.queueNo)
                    )
                  ) {
                    merged.unshift(opt);
                  }
              ...
              console.debug("[overview] optimistic cache merge skipped", e);
            }
            customerInfo.value = merged;
            saveCache();
          }
        } catch (e) {
          console.error("[CurrentOverview] initial map failed", e);
        }
        ...
        try {
          ...
          console.debug("[overview] served list refresh failed", e);
        }
        ...
        try {
          ...
          console.debug("[overview] cancelled list refresh failed", e);
        }
      } catch (e) {
        console.debug("[overview] initial load error", e);
      }

      try {
        connectRealtime();
      } catch (e) {
        console.debug("[overview] realtime connect skipped", e);
      }
      ...
      const refreshAll = async () => {
        try {
          ...
          console.debug("[overview] loadCurrent refresh failed", e);
        }
        try {
          await loadServed();
        } catch (e) {
          console.debug("[overview] loadServed refresh failed", e);
        }
        try {
          await loadCancelled();
        } catch (e) {
          console.debug("[overview] loadCancelled refresh failed", e);
        }
      };

      try {
        unsubs.push(
          onRealtime("queue:joined", (payload) => {
            try {
              const bizIdStr = businessId ? String(businessId) : null;
              const evtBizStr =
                payload && payload.business_id != null
                  ? String(payload.business_id)
                  : null;
              if (bizIdStr && evtBizStr && bizIdStr !== evtBizStr) {
                console.debug(
                  "[overview] queue:joined ignored for different business"
                );
              } else {
                const newId =
                  payload && payload.id != null ? Number(payload.id) : null;
                const newQn =
                  payload && payload.queue_number != null
                    ? String(payload.queue_number).padStart(4, "0")
                    : null;
                const exists = (customerInfo.value || []).some(
                  (c) =>
                    (newId != null && c.id === newId) ||
                    (newQn && c.queueNo === newQn)
                );
                if (!exists) {
                  ...
                  customerInfo.value.unshift({
                    ...
                    paid:
                      String(payload?.payment_status || "pending") === "paid",
                    partySize:
                      payload?.party_size != null
                        ? Number(payload.party_size)
                        : null,
                    optimistic: true,
                  });
                  ...
                  try {
                    ...
                    console.debug("[overview] cache write skipped");
                  }
                }
              }
            } catch (_) {
              console.debug(
                "[overview] queue:joined optimistic insert skipped"
              );
            }
        ...
        console.debug("[overview] queue:joined subscription failed", e);
      }
      try {
        unsubs.push(
          onRealtime("queue:status", (payload) => {
            try {
              ...
              if (String(payload.status || "").toLowerCase() === "delayed") {
                const idx = (customerInfo.value || []).findIndex(
                  (c) => c.id === Number(payload.id)
                );
                if (idx >= 0) {
                  const row = customerInfo.value[idx];
                  const newEta =
                    payload.estimated_wait_time != null
                      ? Number(payload.estimated_wait_time)
                      : row.etaBase;
                  const paid =
                    row.paid ||
                  ...
                  pinnedMap.set(Number(payload.id), {
                    rowSnapshot: customerInfo.value[idx],
                    expiresAtMs: Date.now() + 10000,
                  });

                  lastActiveMap.set(Number(payload.id), {
                    rowSnapshot: { ...customerInfo.value[idx] },
                    lastSeenMs: Date.now(),
                  });
                ...
                String(payload.status || "").toLowerCase() === "called"
              ) {
                const idx = (customerInfo.value || []).findIndex(
                  (c) => c.id === Number(payload.id)
                );
                if (idx >= 0) {
                  ...
                  if (typeof frozen !== "number") {
                    ...
                    if (
                      ...
                      const elapsedMin =
                        (Date.now() - Number(row.initialStart)) / 60000;
                      frozen = Math.max(
                        0,
                        Math.ceil((Number(row.initialEwt) - elapsedMin) * 60)
                      );
                    } else if (row.etaBase != null) {
                      frozen = Math.max(
                        0,
                        Math.round(Number(row.etaBase) * 60)
                      );
                    } else {
                      frozen = 0;
                    }
                ...
                String(payload.status || "").toLowerCase() === "served" ||
                String(payload.status || "").toLowerCase() === "cancelled"
              ) {
                try {
                  lastActiveMap.delete(Number(payload.id));
                  pinnedMap.delete(Number(payload.id));
                } catch (_) {
                  console.debug("[overview] status cleanup skipped");
                }
              }
            } catch (_) {
              console.debug("[overview] queue:status processing skipped");
            }
            refreshAll();
          })
        );
      } catch (e) {
        console.debug("[overview] queue:status subscription failed", e);
      }

      try {
        unsubs.push(
          onRealtime("queue:updated", (ev) => {
            try {
              ...
              if (idx >= 0 && ev.estimated_wait_time != null) {
                ...
                const canTick =
                  (st === "waiting" || st === "delayed") &&
                  (props.isServiceBased ? true : !!row.paid);
                customerInfo.value[idx] = {
                  ...row,
                  etaBase: eta,
                  initialEwt: eta,
                ...
                try {
                  lastActiveMap.set(idNum, {
                    rowSnapshot: { ...customerInfo.value[idx] },
                    lastSeenMs: Date.now(),
                  });
                } catch (_) {
                  console.debug("[overview] lease refresh skipped");
                }
                ...
                try {
                  if (
                    ...
                    selectedOrder.value &&
                    (Number(selectedOrder.value.id) === idNum ||
                      String(selectedOrder.value.queueNo) ===
                        String(row.queueNo))
                  ) {
                    const cur = selectedOrder.value;
                    const merged = {
                  ...
                  console.debug("[overview] selectedOrder ETA sync skipped");
                }
              }
            } catch (_) {
              console.debug("[overview] queue:updated processing skipped");
            }
          })
        );
      } catch (_) {
        console.debug("[overview] queue:updated subscription failed");
      }
      try {
        unsubs.push(onRealtime("settings:updated", refreshAll));
      } catch (e) {
        console.debug("[overview] settings subscription failed", e);
      }
      ...
      try {
        ...
        pollTimer = setInterval(async () => {
          try {
            ...
            console.debug("[overview] poll loadCurrent failed");
          }
        }, 5000);
      } catch (_) {
        console.debug("[overview] poll setup failed");
      }
    });
    onUnmounted(() => {
      ...
      try {
        unsubs.forEach((fn) => {
          try {
            ...
            console.debug("[overview] unsubscribe failed", e);
          }
        });
      } catch (e) {
    ...
    const remainingSeconds = (row) => {
      ...
      const base =
        row.initialEwt != null
          ? Number(row.initialEwt)
          : typeof row.etaBase === "number"
          ? row.etaBase
          : null;
      ...
      if (
        (st === "waiting" || st === "delayed") &&
        (props.isServiceBased ? true : isPaid) &&
        start
      ) {
      ...
      if (typeof row.frozenWaitSeconds === "number")
        return Math.max(0, Math.round(row.frozenWaitSeconds));
    ...
    function buildSummary() {
      const waiting = (customerInfo.value || []).filter(
        (c) => String(c.status).toLowerCase() === "waiting"
      );
      const waitsSec = waiting
        .map(remainingSeconds)
        .filter((v) => typeof v === "number" && !isNaN(v));
      const avgSec = waitsSec.length
        ? Math.round(waitsSec.reduce((a, b) => a + b, 0) / waitsSec.length)
        : 0;

      const avgWait = Math.round(avgSec / 60);
      const completedToday = (completedCustomer.value || []).filter((c) =>
        isTodayDate(c.completedAt)
      ).length;
      const cancelledToday = (cancelledCustomer.value || []).filter((c) =>
        isTodayDate(c.completedAt)
      ).length;
      const waitingPaid = (customerInfo.value || []).filter(
        (c) => String(c.status).toLowerCase() === "waiting" && !!c.paid
      ).length;
    ...
    function emitSummary() {
      try {
        ...
        console.debug("[overview] summary emit skipped");
      }
    }
    ...
    function emitBaselines() {
      try {
        emit("baselines", buildBaselines());
      } catch (_) {
        console.debug("[overview] baselines emit skipped");
      }
    }

    watch(customerTab, (tab) => {
      try {
        localStorage.setItem("overviewActiveTab", tab);
      } catch (e) {
        console.debug("[overview] active tab persist skipped", e);
      }
    });
    ...
    const addCustomer = async (form) => {
      const durationMap = new Map(
        (menuItems.value || []).map((m) => [
          m.id,
          m.duration != null ? Number(m.duration) : null,
        ])
      );
      ...
      const order_total = order_items.reduce(
      ...
      try {
        ...
        const dup = (customerInfo.value || []).some(
          (c) =>
            (c.id && c.id === result.id) || (c.queueNo && c.queueNo === qnStr)
        );
        if (!dup)
          customerInfo.value.unshift({
            id: result.id,
        ...
        try {
          ...
          console.debug("[overview] cache write skipped", e);
        }
    ...
    const handleOrderDetails = async (customer) => {
      const needsHydration =
        !customer ||
        customer.optimistic === true ||
        !customer.customerName ||
        !Array.isArray(customer.menuItems) ||
      ...
      if (needsHydration) {
        try {
          ...
          console.debug("[overview] hydrate current failed");
        }
        if (customer) {
          const byId =
            customer.id != null
              ? customerInfo.value.find((c) => c.id === customer.id)
              : null;
          const byQn =
            !byId && customer.queueNo
              ? customerInfo.value.find((c) => c.queueNo === customer.queueNo)
              : null;
    ...
    const completeCustomer = async (row) => {
      ...
      if (!row || isServeDisabled(row)) {
        ...
        const msg =
          s === "waiting"
            ? "Please call the order before serving"
            : "Cannot mark as served while not called";
        return Swal.fire({
          toast: true,
          position: "top-end",
          icon: "info",
      ...
      const index = customerInfo.value.findIndex(
        (c) => row && (c.id === row.id || c.queueNo === row.queueNo)
      );
      const customer = index >= 0 ? customerInfo.value[index] : null;
      if (!customer) return;
      const res = await Swal.fire({
        title: props.isServiceBased
          ? "Set to Pending Payment?"
          : "Mark as served?",
        text: props.isServiceBased
          ? "Customer will proceed to payment. Entry stays in Current until payment completes."
          : "This will move the order to Completed.",
        icon: "question",
      ...
      try {
        if (props.isServiceBased) {
          await apiUpdateQueueStatus(customer.id, {
            status: "pending_payment",
          });

          customerInfo.value[index] = {
            ...customer,
        ...
        return Swal.fire({
          ...
          position: "top-end",
          icon: "error",
          title: e?.message || "Failed to mark as served",
          timer: 2000,
          timerProgressBar: true,
          showConfirmButton: false,
        });
      ...
      if (!props.isServiceBased) {
        const moved = {
          ...customer,
          status: "Completed",
          payment: customer.paid ? "Paid" : "Pending",
        ...
        try {
          ...
          console.debug("[overview] loadServed refresh failed");
        }
      }
      completing = false;
      if (!props.isServiceBased)
    ...
    const filteredCustomers = computed(() => {
      ...
      return customerInfo.value.filter(
        (customer) =>
          String(customer.customerName || "")
            .toLowerCase()
            .includes(q) || String(customer.queueNo || "").includes(q)
      );
    });

    const currentTotalPages = computed(() =>
      Math.max(1, Math.ceil((filteredCustomers.value?.length || 0) / PAGE_SIZE))
    );

    const filteredCompleted = computed(() => {
      if (!searchQuery.value) return completedCustomer.value;
      const q = String(searchQuery.value || "").toLowerCase();
      return completedCustomer.value.filter(
        (c) =>
          String(c.customerName || "")
            .toLowerCase()
            .includes(q) || String(c.queueNo || "").includes(q)
      );
    });
    const filteredCancelled = computed(() => {
      if (!searchQuery.value) return cancelledCustomer.value;
      const q = String(searchQuery.value || "").toLowerCase();
      return cancelledCustomer.value.filter(
        (c) =>
          String(c.customerName || "")
            .toLowerCase()
            .includes(q) || String(c.queueNo || "").includes(q)
    ...
    const completedTotalPages = computed(() =>
      Math.max(1, Math.ceil((filteredCompleted.value?.length || 0) / PAGE_SIZE))
    );
    const cancelledTotalPages = computed(() =>
      Math.max(1, Math.ceil((filteredCancelled.value?.length || 0) / PAGE_SIZE))
    );
    const currentPages = computed(() =>
      Array.from({ length: currentTotalPages.value }, (_, i) => i + 1)
    );
    const completedPages = computed(() =>
      Array.from({ length: completedTotalPages.value }, (_, i) => i + 1)
    );
    const cancelledPages = computed(() =>
      Array.from({ length: cancelledTotalPages.value }, (_, i) => i + 1)
    );
    ...
    watch(filteredCustomers, () => {
      const total = Math.max(
        1,
        Math.ceil((filteredCustomers.value?.length || 0) / PAGE_SIZE)
      );
    ...
    watch([searchQuery, () => completedCustomer.value.length], () => {
      const total = Math.max(
        1,
        Math.ceil((filteredCompleted.value?.length || 0) / PAGE_SIZE)
      );
      if (currentPageCompleted.value > total)
        currentPageCompleted.value = total;
      if (currentPageCompleted.value < 1) currentPageCompleted.value = 1;
    });
    watch([searchQuery, () => cancelledCustomer.value.length], () => {
      const total = Math.max(
        1,
        Math.ceil((filteredCancelled.value?.length || 0) / PAGE_SIZE)
      );
      if (currentPageCancelled.value > total)
        currentPageCancelled.value = total;
    ...
    const handleUpdateCustomer = (updateCustomer) => {
      ...
      const idx = customerInfo.value.findIndex(
        (c) =>
          (selectedOrder.value.id && c.id === selectedOrder.value.id) ||
          c.queueNo === selectedOrder.value.queueNo
      );
      if (idx !== -1) {
        const row = customerInfo.value[idx];
        ...
        if (Array.isArray(updateCustomer.menuItems))
          row.menuItems = updateCustomer.menuItems;
        if (typeof updateCustomer.isPriority === "boolean")
          row.isPriority = updateCustomer.isPriority;
        if (updateCustomer.option) row.option = updateCustomer.option;
        if (updateCustomer.partySize != null)
          row.partySize = updateCustomer.partySize;
      }
    };

    const handleStatusChanged = ({ id, status, paid }) => {
      const idx = customerInfo.value.findIndex(
        (c) => c.id === id || c.queueNo === selectedOrder.value.queueNo
      );
      if (idx !== -1) {
        if (status === "served" || status === "cancelled") {
          ...
          customerInfo.value[idx].status =
            status === "called"
              ...
              ? "Pending Payment"
              : status;
          if (status === "waiting" || paid === true)
            customerInfo.value[idx].paid = true;

          if (status === "waiting")
            customerInfo.value[idx].etaUpdated = Date.now();
        }
      }
    ...
    const waitingPaidCount = computed(
      () =>
        (customerInfo.value || []).filter(
          (c) => String(c.status).toLowerCase() === "waiting" && !!c.paid
        ).length
    );
    const waitingCount = computed(
      () =>
        (customerInfo.value || []).filter((c) => {
    ...
    const addDisabled = computed(() => {
      ...
      return (
        (props.isServiceBased ? waitingCount.value : waitingPaidCount.value) >=
        maxLen
      );
    });

<template>
  <ion-modal
    :is-open="isOpen"
    class="custom-modal call-customers-modal"
    :backdrop-dismiss="false"
  >
    <div id="inter" class="bg-[#283618] text-white p-3 rounded-lg w-full">
      <div class="flex items-center justify-between mb-3">
        <div>
          <p class="text-2xl font-bold">Call Customers</p>
          <p class="text-sm font-light opacity-90">
            Select customers currently waiting and paid, then call them. EWT
            mirrors the Current Queue countdown.
          </p>
        </div>
        <font-awesome-icon
          :icon="['fas', 'xmark']"
      ...
      <div class="mb-3">
        <div class="flex items-center gap-2 mb-2 w-full">
          <ion-input
            ...
            class="bg-white/20 rounded-sm text-white text-sm w-full"
            style="
          ...
          <ion-button
            size="small"
            fill="clear"
            class="bg-[#283618] text-white normal-case border border-white/30 py-1 hover:opacity-75"
            @click="load"
            >Refresh</ion-button
          >
        </div>
        <div
          class="border border-[#DDA15E] rounded-sm max-h-[60vh] overflow-y-auto w-full"
        >
            ...
            <tbody>
              ...
              >
                <td class="px-2 py-2 text-center">
                  <label
                    class="checkbox-wrap"
                    :aria-label="`Select queue ${row.queue_number}`"
                  >
                    <input
                      type="checkbox"
                      class="big-checkbox"
              ...
              <tr v-if="!loading && !filtered.length">
                <td colspan="6" class="text-center text-gray-300 py-4">
                  No eligible waiting customers
                </td>
              </tr>
              <tr v-if="loading">
                <td colspan="6" class="text-center text-gray-300 py-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 items-center gap-2 mt-3">
        <div class="text-xs text-gray-200 md:col-span-1">
          Selected: {{ selected.length }}
        </div>
        <div class="md:col-span-2 flex gap-2 justify-end">
          <ion-button
            fill="clear"
            class="bg-gray-500 normal-case w-full md:w-auto"
            @click="$emit('close')"
            >Cancel</ion-button
          >
          <ion-button
            fill="clear"
            class="bg-[#DDA15E] text-white normal-case w-full md:w-auto"
            :disabled="!selected.length || submitting"
            @click="submit"
            >{{ submitting ? "Calling..." : "Call Selected" }}</ion-button
          >
        </div>
      </div>
    </div>
  ...
  setup(props, { emit }) {
    ...
    const saveStartsToCache = () => {
      try {
        sessionStorage.setItem(
          CACHE_KEY,
          JSON.stringify(localStarts.value || {})
        );
      } catch (err) {
    ...
    const saveFrozenToCache = () => {
      try {
        sessionStorage.setItem(
          FROZEN_KEY,
          JSON.stringify(frozenSeconds.value || {})
        );
      } catch (err) {
    ...
    const allSelected = computed(
      () =>
        ...
        filtered.value.every((r) => selected.value.includes(r.id))
    );
    ...
    const getRemainingSeconds = (id) => {
      if (!id) return 0;
      if (frozenSeconds.value?.[id] != null)
        return Number(frozenSeconds.value[id]) || 0;
      const parentBaseline = props.baselines?.[id];
      if (
        parentBaseline &&
        parentBaseline.initialEwt != null &&
        parentBaseline.initialStart != null
      ) {
        const rawMin =
          Number(parentBaseline.initialEwt) -
          (now.value.getTime() - Number(parentBaseline.initialStart)) / 60000;
        return Math.max(0, Math.ceil(rawMin * 60));
      }
      const r = rows.value.find((rr) => Number(rr.id) === Number(id));
      const baseMin =
        r?.estimated_wait_time != null ? Number(r.estimated_wait_time) : 0;
    ...
    const submit = async () => {
      ...
      try {
        ...
        if (skipped.length)
          toast(`${skipped.length} skipped (not eligible)`, "warn");
        if (result.capacity) {
          const cap = result.capacity;
          toast(
            `Capacity: staff=${cap.staff}, in-use=${cap.in_use}, remaining=${cap.remaining}`
          );
        }
        try {
          const ids = updated
      ...
      (open) => {
        if (open) {
          ...
          try {
            ...
            console.debug(
              "[call-modal] realtime connect failed (open watcher)",
              e
            );
          }
          try {
            offStatus = onRealtime("queue:status", (ev) => {
              ...
              if (status === "delayed") {
                ...
                if (idx >= 0) {
                  const eta =
                    ev.estimated_wait_time != null
                      ? Number(ev.estimated_wait_time)
                      : rows.value[idx].estimated_wait_time;
                  rows.value[idx] = {
                    ...rows.value[idx],
                    status: "delayed",
            ...
            offUpdated = onRealtime("queue:updated", (ev) => {
              ...
              if (idx >= 0 && ev.estimated_wait_time != null) {
                if (
                  frozenSeconds.value?.[id] != null ||
                  String(rows.value[idx].status).toLowerCase() === "called"
                ) {
                  return;
                }
          ...
          try {
            ...
            console.debug(
              "[call-modal] saveStartsToCache failed on close",
              err
            );
          }
          try {
            ...
            console.debug(
              "[call-modal] saveFrozenToCache failed on close",
              err
            );
          }
        }

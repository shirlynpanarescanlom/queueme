  ...
  <div class="text-white bg-[#283618] p-3 rounded-lg mx-4 mb-8 md:w-[860px]">
    <div class="flex justify-between items-center mb-2">
      <div>
        ...
        <p id="poppins" class="font-light text-sm">
          Add your initial staff and configure your queue
        </p>
      </div>
    </div>
    ...
    <ion-segment class="bg-white/10 rounded mb-4 w-full md:w-auto">
      <ion-segment-button
        ...
        :class="
          activeTab === 'staff' ? 'bg-[#606C38] text-white' : 'text-white'
        "
      >
      ...
      <ion-segment-button
        ...
        :class="
          activeTab === 'queue' ? 'bg-[#606C38] text-white' : 'text-white'
        "
      >
    ...
    <div v-if="activeTab === 'staff'" class="grid md:grid-cols-2 gap-4">
      <div class="border border-white/20 rounded-md p-3">
        ...
        >
          ...
          <ion-button
            fill="clear"
            size="small"
            class="bg-[#BC6C25] normal-case"
            @click="beginAdd"
            :disabled="editing"
            >Add</ion-button
          >
        </h3>
        <div v-if="!staffList.length" class="text-xs text-gray-300">
          No staff added yet.
        </div>
        <ul v-else class="space-y-2 max-h-72 overflow-y-auto pr-1 text-xs">
          <li
            ...
            :key="s.tempId"
            class="bg-white/5 rounded px-2 py-2 flex items-start justify-between gap-2"
          >
            <div class="leading-tight">
              <p class="font-semibold">
                {{ s.name }}
                <span
                  class="text-[10px] uppercase tracking-wide bg-black/30 px-1 rounded"
                  >{{ s.role }}</span
                >
              </p>
            ...
            <div class="flex gap-1">
              ...
              <ion-button
                ...
                size="small"
                color="danger"
                class="bg-red-700 normal-case"
                @click="removeStaff(s)"
                :disabled="editing || s.role === 'owner'"
                >Del</ion-button
              >
            </div>
          </li>
      ...
      <div class="border border-white/20 rounded-md p-3" v-if="editing">
        <h4 id="poppins" class="font-semibold mb-2">
          {{ editMode === "add" ? "Add Staff" : "Edit Staff" }}
        </h4>
        <form @submit.prevent="submitStaff" class="space-y-2 text-xs">
          <div class="flex flex-col">
            ...
            <input
              ...
              :maxlength="10"
              @blur="touchedStaff.name = true"
              required
              class="rounded text-black px-2 py-1 text-xs"
            />
            <p class="text-[11px] text-white/70 self-end mt-1">
              {{ (staffForm.name || "").length }} / 10
            </p>
            <p
              v-if="
                (touchedStaff.name || staffSubmitAttempted) && staffErrors.name
              "
          ...
          <div class="flex flex-col">
            ...
            <p
              v-if="
                (touchedStaff.email || staffSubmitAttempted) &&
                staffErrors.email
              "
              class="text-red-300 mt-1"
            >
          ...
          >
            <label
              >Password
              <span v-if="editMode === 'add'" class="text-red-400"
                >*</span
              ></label
            >
            <input
              v-model="staffForm.password"
              @blur="touchedStaff.password = true"
            ...
            <p
              v-if="
                (touchedStaff.password || staffSubmitAttempted) &&
            ...
            <p v-if="editMode === 'edit'" class="text-[10px] opacity-80">
    ...
    <div v-else class="grid md:grid-cols-1 gap-4">
      ...
      <div class="border border-white/20 rounded-md p-3 bg-white/5">
        <label id="poppins" class="block font-semibold mb-2"
          >Business Registration Proof
          <span class="text-red-300">*</span></label
        >
        <p class="text-[12px] opacity-80 mb-3">
          Upload a clear photo or PDF of your DTI/SEC Certificate or business
          permit.
        </p>
        <div class="flex items-center gap-2">
          <input
            ...
            @change="onFileChange"
            class="text-xs file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:bg-[#DDA15E] file:text-white file:cursor-pointer"
          />
          <span v-if="fileName" class="text-[12px] opacity-90">{{
            fileName
          }}</span>
        </div>
        <p v-if="fileError" class="text-[12px] mt-2 text-red-300">
          {{ fileError }}
        </p>
      </div>
    </div>
    ...
    <div class="mt-4 flex flex-col md:flex-row gap-2">
      <ion-button
        ...
        fill="clear"
        class="md:w-auto w-full bg-[#DDA15E] rounded-sm normal-case text-white text-base"
      ...
      <ion-button
        ...
        @click="goBack"
        fill="clear"
        class="md:w-auto w-full normal-case text-white border border-[#DDA15E] rounded-sm text-base"
        >Back</ion-button
      >
    </div>
    <p class="text-xs mt-2 text-red-300" v-if="!staffList.length">
      At least one staff (owner) is required.
    </p>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    submitStaff() {
      ...
      if (
        ...
        (this.editMode === "add" && !this.staffForm.password)
      )
        return;
      if (this.editMode === "add") {
        this.staffList.push({ ...this.staffForm });
      } else {
        const idx = this.staffList.findIndex(
          (s) => s.tempId === this.staffForm.tempId
        );
    ...
    finish() {
      ...
      const mappedStaff = this.staffList.map((s) => ({
        name: s.name,
        email: s.email,
        password: s.password,
        role: s.role,
    ...
    onFileChange(e) {
      ...
      if (!okType) {
        this.fileError = "File must be JPG, PNG, or PDF";
        this.proofFile = null;
        this.fileName = "";
        return;
      }
      if (!okSize) {
        this.fileError = "File must be 5MB or less";
        this.proofFile = null;
        this.fileName = "";
        return;
      }
  ...
  computed: {
    staffErrors() {
      ...
      else if (String(this.staffForm.name).trim().length > 10)
        e.name = "Name must be at most 10 characters";
      const email = String(this.staffForm.email || "");
      if (!email) e.email = "Email is required";
      else if (/\s/.test(email)) e.email = "Email cannot contain spaces";
      if (this.editMode === "add" && !this.staffForm.password)

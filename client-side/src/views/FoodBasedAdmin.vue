  ...
  <ion-page>
    <ion-header>
      <ion-toolbar>
        <div class="logo flex items-center justify-between mx-3">
          ...
          <div
            class="text-[#1f2937] bg-white/80 px-2 py-0.5 rounded-sm border border-gray-200 shadow-sm"
          >
            {{ currentTime }}
          </div>
            ...
            <div>
              ...
              <ion-popover trigger="click-trigger" trigger-action="click">
                <ion-content>
                  ...
                  <div
                    v-if="role !== 'cashier'"
                    class="border-b border-t text-[#283618]"
                  >
                    <ion-button
                      @click="notifications"
                      fill="clear"
    ...
    <ion-content fullscreen>
      ...
      <div v-if="activeTab === 'overview'">
        <div class="mx-3">
          <Overview1
            v-show="showOverview1"
            :metrics="
              overviewMetrics || {
                totalCustomers: overviewSummary.totalCustomers,
                avgWait: overviewSummary.avgWait,
                completedToday: overviewSummary.completedToday,
                cancelledToday: overviewSummary.cancelledToday,
              }
            "
          />
          <div
            v-if="webhookBlink"
            class="mt-2 text-xs px-2 py-1 inline-flex items-center gap-2 rounded bg-emerald-50 text-emerald-700 border border-emerald-200"
          >
          ...
          <div class="my-4" v-if="settingsSummary">
            <div
              class="border border-gray-200 rounded-sm p-3 bg-[#f8f8f8] text-[#283618] text-xs flex gap-6 flex-wrap"
            >
              <div>
                <span class="font-semibold">Max Queue:</span>
                {{ settingsSummary.maxQueueLength }}
              </div>
              <div>
                <span class="font-semibold">Reserve Slots:</span>
                {{ settingsSummary.reserveSlots }}
              </div>
              <div>
                <span class="font-semibold">Notify Customer:</span>
                {{ settingsSummary.notifyCustomer ? "Yes" : "No" }}
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-3 gap-3 mx-3 mb-5">
          <CurrentOverview
            v-show="showCurrentOverview"
            :settings-summary="settingsSummary"
            @summary="onOverviewSummary"
            @baselines="onBaselines"
          />
          <RecentActivity />
        </div>
      </div>
      ...
      <div v-else-if="activeTab === 'queueManagement'">
        <div class="mx-3 mb-2" v-if="settingsSummary">
          <p class="text-[11px] text-gray-500">
            Configured Max:
            <strong>{{ settingsSummary.maxQueueLength }}</strong> | Reserve
            Slots: <strong>{{ settingsSummary.reserveSlots }}</strong> | Notify:
            <strong>{{ settingsSummary.notifyCustomer ? "On" : "Off" }}</strong>
          </p>
        </div>
        <div class="grid grid-cols-3 my-5 mx-3 gap-4">
          <div class="col-span-3 lg:col-span-1" id="poppins">
            <div class="rounded-lg border border-gray-200 bg-white p-4">
              <p class="text-2xl font-semibold text-[#283618] mb-3">
                Queue Summary
              </p>
              <div class="grid grid-cols-2 gap-3">
                <div
                  class="rounded-lg p-4 bg-[#FFFCDF] border border-[#f0eec6] text-center text-[#283618]"
                >
                  <p class="font-semibold mb-2">In Queue</p>
                  <div
                    class="flex items-center justify-center gap-2 text-3xl font-bold"
                  >
                    <font-awesome-icon :icon="['fas', 'users']" />
                    <span>{{
                      (queueSummary && queueSummary.inQueue) ??
                ...
                >
                  ...
                  >
                    ...
                    <span>{{
                      (queueSummary && queueSummary.cancelled) ??
                      overviewSummary.cancelledToday
                    }}</span>
                  </div>
                </div>
                <div
                ...
                >
                  ...
                  >
                    ...
                    <span>{{
                      (queueSummary && queueSummary.avgWait) ??
                      overviewSummary.avgWait
                    }}</span>
                  </div>
                </div>
                <div
                ...
                >
                  ...
                  >
                    ...
                    <span>{{
                      (queueSummary && queueSummary.completedToday) ??
                      overviewSummary.completedToday
                    }}</span>
                  </div>
                </div>
              </div>
              <div class="mt-4">
                <ion-button
                  expand="block"
                  class="normal-case bg-[#283618] hover:opacity-90"
...
export default {
  ...
  setup() {
    ...
    function onOverviewSummary(s) {
      overviewSummary.value = s || overviewSummary.value;
    }
    ...
    try {
      ...
      const allowed =
        role.value === "cashier"
          ? ["overview", "queueManagement"]
          : ["overview", "queueManagement", "menu", "analytics"];

      if (savedAdminTab && allowed.includes(savedAdminTab))
        activeTab.value = savedAdminTab;
      else activeTab.value = allowed[0];
    } catch (e) {
      console.debug(
        "[food-admin] failed to access localStorage for adminActiveTab",
        e
      );
    }
    watch(activeTab, (tab) => {
      try {
        ...
        console.debug("[food-admin] could not persist adminActiveTab", e);
      }
    });
    ...
    try {
      ...
      console.debug("[food-admin] realtime connect failed", err);
    }
    try {
      onRealtime("payment:webhook", (ev) => {
        try {
          ...
          if (ev && Number(ev.business_id) === myBiz) {
            ...
            setTimeout(() => {
              webhookBlink.value = false;
            }, 3000);
          }
        } catch (err) {
          console.debug("[food-admin] operation failed", err);
        }
      });
    } catch (err) {
      console.debug("[food-admin] error during cleanup", err);
    }
